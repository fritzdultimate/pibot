...
const UNLOCK_TIME_STRING = "2025-05-15 19:57:16"
...
async function preSignTxs() {
    const account = await server.loadAccount(sourceKeypair.publicKey());
    let sequence = BigInt(account.sequence);

    for (let i = 0; i < MAX_REAL_TXS; i++) {
        const tx = new stellarSDK.TransactionBuilder(account, {
        fee: (10000 + i * 10000).toString(), // Optional: increasing fee
        networkPassphrase: "Pi Network",
        })
        .addOperation(stellarSDK.Operation.payment({
            destination: ONTESTNET ? RECEIVER_TEST : RECEIVER_MAIN,
            asset: stellarSDK.Asset.native(),
            amount: AMOUNT_TO_WITHDRAW.toString(),
        }))
        .setTimeout(30)
        .build();

        tx.sequence = (sequence + BigInt(i + 1)).toString();
